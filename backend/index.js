...
app.post("/paymentverfication", async (req, res) => {
  const { razorpay_order_id, razorpay_paymet_id, razorpay_signature } =
    req.body;
  const body = razorpay_order_id + "|" + razorpay_paymet_id;
  ...
  if (isauth) {
    ...
    res.redirect(
      `http://localhost:5173/paymentsucess?reference=${razorpay_order_id}`
    );
  } else {
